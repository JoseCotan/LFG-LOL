...
const MensajeSuccess = ({ message, onClose }) => {
    return (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 mt-4 rounded relative mb-4 max-w-xs" role="alert" style={{ maxWidth: '400px' }}>
            ...
            <span className="block sm:inline ml-4 mr-6">{message}</span>
            <span className="absolute top-0 bottom-0 right-0 px-4 py-3" onClick={onClose}>
                <svg className="fill-current h-6 w-6 text-green-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 20">
                    <title>Close</title>
